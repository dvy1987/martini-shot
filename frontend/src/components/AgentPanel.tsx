    ...
    <section aria-label="How Martini Shot reached this result">
      ...
      <div className="mt-3 rounded-md border border-line bg-surface-2 p-4">
        ...
        {ranked.length > 0 ? (
          <ul className="mt-2 space-y-2">
            {ranked.map((action, index) => (
              <li key={`${action.command_name}-${index}`} className="text-sm text-ink">
                <span className="font-mono text-xs">{action.command_name.replaceAll("_", " ")}</span>
                ...
                <span className="ml-2 font-mono text-xs uppercase tracking-wider text-ink-muted">
                  {action.reversible ? "Can be undone" : "Cannot be undone automatically"}
                </span>
              </li>
            ))}
          </ul>
          ...
          <p className="mt-2 text-sm text-ink-muted">
            No further action was suggested for this job.
          </p>
        )}
      </div>
      ...
      {dissent.length > 0 ? (
        <div className="mt-3 rounded-md border border-danger/40 bg-surface-2 p-4">
          <p className="font-mono text-xs uppercase tracking-wider text-danger">Different opinions</p>
