...
| 3 | Small + capped instances | Backend deploys with `--cpu=1 --memory=2Gi --max-instances=3` in the current `deploy.sh`. The frontend is a Vite bundle served by the selected host or combined deployment. Media work runs through the backend worker and FFmpeg. | `deploy.sh` |
...
| 7 | Secure endpoints | Public health is open. Other API routes require the configured API key. Owner settings writes require a Google Identity Services ID token verified against the owner allowlist. Secrets stay outside the repository. | ✅ `backend/api/app.py`, `frontend/src/components/GoogleSignInButton.tsx` |
