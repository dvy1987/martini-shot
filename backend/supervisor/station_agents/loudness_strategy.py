...
def build_prompt(report: dict[str, Any], season_state: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "ambience in family with that previous mix — compare dialogue levels "
        "against the previous shot's dialogue band so voices remain balanced "
        "across the cut. If dialogue drops noticeably below the previous shot's "
        "dialogue or is covered by ambience, choose fix_stem to lift the voice. "
