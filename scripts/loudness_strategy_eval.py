#!/usr/bin/env python
"""Loudness strategy eval (A10-3, C-3.3/.4): the REAL agent decides the
remediation path and routing for each seeded loudness report, with season
state in context; scored against the expected decision.

Gate (thresholds.yaml): loudness_strategy_judgment mean_case_accuracy >= 0.8.
Honest accounting (C-3.5): a failed/invalid decision counts as a MISS.

Evidence: docs/evidence/A10-3/loudness_strategy_eval.jsonl + _summary.json.
...
DATASET = ROOT / "backend" / "evals" / "datasets" / "loudness_strategy_judgment.jsonl"
...
def run_suite(run_index: int) -> tuple[list[dict], dict]:
    ...
    for row in rows:
        ...
        try:
            doc, _cost = decide_loudness_strategy(
                settings,
                report=row["report"],
...
def main() -> int:
    ...
    for i in range(1, args.runs + 1):
        print(f"=== loudness_strategy_judgment run {i}/{args.runs} ===", flush=True)
    ...
    payload = {
        "suite": "loudness_strategy_judgment",
        "runs": args.runs,
    ...
    (EVIDENCE / "loudness_strategy_eval.jsonl").write_text(
        "\n".join(json.dumps(r) for r in all_records) + "\n", encoding="utf-8"
    )
    (EVIDENCE / "loudness_strategy_summary.json").write_text(
